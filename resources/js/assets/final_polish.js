...
const payloadRegex = /const payload = \{\s*working_days: editDoc\.workingDays,\s*day_shifts: editDoc\.dayShifts,\s*shift_type: editDoc\.shiftType \|\| editDoc\.dayShifts\?\.\[0\]\?\.type \|\| 'morning'\s*\};/;
...
content = content.replace(offDayRegex, newOffDay);

// 3. Enhance Update Button
const updateBtnRegex = /<button \s*onClick=\{handleBatchSave\}\s*className="flex-1 bg-primary-600 text-white py-4 rounded-2xl text-\[11px\] font-black uppercase tracking-widest hover:bg-primary-700 hover:shadow-xl hover:shadow-primary-100 hover:-translate-y-0\.5 transition-all flex items-center justify-center gap-3 shadow-lg"\s*>\s*<Activity size=\{18\} \/>\s*\{isAr \? 'تعديل البيانات' : 'Update Physician'\}\s*<\/button>/;
...
content = content.replace(updateBtnRegex, newUpdateBtn);
