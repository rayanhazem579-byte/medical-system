...
export const DoctorsPage: FC<DoctorsPageProps> = ({
  ...
  const filteredDoctors = doctors.filter(doc => {
    ...
    return (
      ...
      (doc.specialtyAr || doc.specialtyEn || '').toLowerCase().includes(searchLow) ||
      (doc.medical_id || '').toLowerCase().includes(searchLow)
    );
  });

  const toggleDay = (day: string) => {
    const currentDays = newDoctor.day_shifts || [];
    const exists = currentDays.find((d: any) => d.day === day);
    if (exists) {
      setNewDoctor({ ...newDoctor, day_shifts: currentDays.filter((d: any) => d.day !== day) });
    } else {
      setNewDoctor({ ...newDoctor, day_shifts: [...currentDays, { day, type: 'morning', work_hours: 8 }] });
    }
  };

  const updateDayShift = (day: string, field: 'type' | 'work_hours', value: any) => {
    const updated = (newDoctor.day_shifts || []).map((d: any) => 
      d.day === day ? { ...d, [field]: value } : d
    );
    setNewDoctor({ ...newDoctor, day_shifts: updated });
  };
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          <div className="relative group">
            ...
            <input 
              type="text" 
              placeholder={isAr ? 'بحث عن طبيب...' : 'Find doctor...'}
          ...
          <button onClick={() => setIsAddDoctorModalOpen(true)} className="px-6 py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-xs font-black flex items-center gap-2 transition-all shadow-lg shadow-primary-100/50 transform hover:scale-[1.02] active:scale-95">
            <Plus size={16} />
            ...
            <table className="w-full text-right" dir={isAr ? 'rtl' : 'ltr'}>
              <thead className="bg-gray-50/50 border-b border-gray-100">
                <tr className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] font-cairo">
                  <th className="px-8 py-5 text-right">{isAr ? 'الطبيب' : 'Physician'}</th>
                  <th className="px-8 py-5 text-center">{isAr ? 'التخصص' : 'Specialty'}</th>
                  ...
                  <tr key={doc.id} className="hover:bg-slate-50/50 transition-colors animate-in fade-in duration-500 font-cairo group">
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-sm font-black text-slate-800 leading-tight">{isAr ? doc.nameAr : doc.nameEn}</p>
                          <p className="text-[10px] text-slate-400 font-bold tracking-widest uppercase mt-0.5">{doc.medical_id || `DOC-${doc.id}`}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-5 text-center">
                      <span className="px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-xl text-[10px] font-black uppercase tracking-tight border border-emerald-100 shadow-sm">{isAr ? doc.specialtyAr : doc.specialtyEn}</span>
                    </td>
                    <td className="px-8 py-5 text-center text-xs font-bold text-slate-600">{isAr ? doc.deptAr : doc.deptEn}</td>
                    <td className="px-8 py-5 text-center">
                       <span className="text-[10px] font-black text-indigo-600">{isAr ? doc.job_title : doc.job_title}</span>
                    </td>
                    <td className="px-8 py-5 text-center">
                    ...
                    <td className="px-8 py-5 text-center">
                      <div className="flex items-center justify-center gap-3">
                          ...
                          <button 
                            onClick={(e) => { e.stopPropagation(); setNewDoctor(doc); setIsAddDoctorModalOpen(true); }}
            ...
            <table className="w-full text-right" dir={isAr ? 'rtl' : 'ltr'}>
              <thead className="bg-gray-50/50 border-b border-gray-100 font-cairo">
                <tr className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                  <th className="px-8 py-5 text-right">{isAr ? 'الطبيب' : 'Doctor'}</th>
                  <th className="px-8 py-5 text-center">{isAr ? 'التخصص' : 'Specialty'}</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {filteredDoctors.map((doc) => (
                  <tr key={doc.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-8 py-5">
                        <p className="text-sm font-black text-slate-800">{isAr ? doc.nameAr : doc.nameEn}</p>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{doc.medical_id || `ID: ${doc.id}`}</p>
                    </td>
                    <td className="px-8 py-5 text-center">
                       <span className="px-3 py-1 bg-white border border-gray-100 text-indigo-600 rounded-lg text-[10px] font-black uppercase tracking-tight shadow-sm">{isAr ? doc.specialtyAr : doc.specialtyEn}</span>
                    </td>
                    <td className="px-8 py-5">
                      <div className="flex flex-wrap gap-1 justify-center">
                        {doc.day_shifts && Array.isArray(doc.day_shifts) ? doc.day_shifts.map((s: any) => (
                    ...
                    <td className="px-8 py-5 text-center">
                       <div className="flex items-center justify-center gap-3">
                          ...
                          <button 
                            onClick={() => { setNewDoctor(doc); setIsAddDoctorModalOpen(true); }}
                 ...
                 <div className="flex flex-col md:flex-row gap-10 items-start">
                    ...
                    <div className="flex-1 space-y-6">
                       <div className="space-y-1">
                          <h2 className="text-4xl font-black text-slate-900 leading-tight">{isAr ? selectedProfileDoctor.nameAr : selectedProfileDoctor.nameEn}</h2>
                          <div className="flex flex-wrap items-center gap-4 mt-2">
                             <span className="px-4 py-1.5 bg-primary-100 text-primary-600 rounded-full text-xs font-black uppercase tracking-widest">{isAr ? selectedProfileDoctor.specialtyAr : selectedProfileDoctor.specialtyEn}</span>
                             <span className="flex items-center gap-2 text-slate-400 text-xs font-bold tracking-widest uppercase"><Hash size={14} /> {selectedProfileDoctor.medical_id || `DOC-${selectedProfileDoctor.id}`}</span>
                          </div>
                       </div>
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          <div className="p-5 rounded-3xl bg-slate-50 border border-slate-100 shadow-sm transition-all hover:shadow-md">
                             ...
                             <p className="text-sm font-black text-slate-800">{isAr ? selectedProfileDoctor.deptAr : selectedProfileDoctor.deptEn}</p>
                          </div>
                          <div className="p-5 rounded-3xl bg-slate-50 border border-slate-100 shadow-sm transition-all hover:shadow-md">
                             <div className="flex items-center gap-3 text-slate-400 mb-2 uppercase tracking-[0.2em] font-black text-[9px]"><DollarSign size={12}/> {isAr ? 'الراتب' : 'Salary Package'}</div>
                             <p className="text-sm font-black text-emerald-600 font-mono tracking-widest">{(parseFloat(selectedProfileDoctor.salary) || 0).toLocaleString()} $</p>
                          </div>
                          <div className="p-5 rounded-3xl bg-slate-50 border border-slate-100 shadow-sm transition-all hover:shadow-md">
                             <div className="flex items-center gap-3 text-slate-400 mb-2 uppercase tracking-[0.2em] font-black text-[9px]"><Calendar size={12}/> {isAr ? 'تاريخ التعيين' : 'Practice Since'}</div>
                             <p className="text-sm font-black text-slate-800 font-mono tracking-widest">{selectedProfileDoctor.hire_date || '2024-01-01'}</p>
                          </div>
                       </div>
                       ...
                       <div className="p-8 rounded-[3rem] bg-indigo-50/50 border-2 border-dashed border-indigo-100">
                          ...
                          <div className="grid grid-cols-4 md:grid-cols-7 gap-4 text-center">
                             {DAYS_SHORT.map(day => {
                                const shift = (selectedProfileDoctor.day_shifts || []).find((s: any) => s.day === day.key || s.day === day.labelAr);
                                return (
                       ...
                       <div className="flex flex-wrap gap-8 items-center bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-inner">
                          <div className="flex items-center gap-4">
                             ...
                             <div>
                                ...
                                <p className="text-xs font-black text-slate-800">{selectedProfileDoctor.email || 'N/A'}</p>
                             </div>
                          </div>
                          <div className="flex items-center gap-4">
                             ...
                             <div>
                                ...
                                <p className="text-xs font-black text-slate-800 font-mono tracking-widest">{selectedProfileDoctor.phone || 'N/A'}</p>
                             </div>
                          </div>
                          <div className="flex items-center gap-4">
                             ...
                             <div>
                                ...
                                <p className="text-xs font-black text-slate-800 truncate max-w-[150px]">{selectedProfileDoctor.bank_name || 'N/A'} - {selectedProfileDoctor.bank_account || 'N/A'}</p>
                             </div>
                          </div>
                       </div>
                    </div>
          ...
          <div className={`relative w-full max-w-4xl bg-white rounded-[3rem] shadow-2xl overflow-hidden border border-white/20 animate-in zoom-in slide-in-from-bottom-4 duration-500 ${isAr ? 'font-cairo' : ''}`}>
            <div className="p-8 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-gray-50 to-white">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xl font-black text-gray-800">{newDoctor.id ? (isAr ? 'تعديل بيانات الطبيب' : 'Edit Physician Details') : (isAr ? 'إضافة طبيب جديد' : 'Onboard New Physician')}</h3>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2 px-1">{isAr ? 'سعر الكشف' : 'Consultation Fee ($)'}</label>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-slate-50 p-6 rounded-[2.5rem] border-2 border-dashed border-slate-200">
                            ...
                            {DAYS_SHORT.map(day => (
                                <button
                                    ...
                                    className={`w-12 h-12 rounded-2xl text-[10px] font-black transition-all transform active:scale-90 flex items-center justify-center border-2 ${
                                        (newDoctor.day_shifts || []).find((d: any) => d.day === (isAr ? day.labelAr : day.key))
                        ...
                        <div className="space-y-4 max-h-[400px] overflow-y-auto px-1 custom-scrollbar">
                           {(newDoctor.day_shifts || []).map((ds: any) => (
                             <div 
                                 key={ds.day}
                                         ...
                                         <div className="flex gap-2">
                                             {SHIFT_TYPES.map((t) => (
                                                 <button
                                                     key={t.id}
                                                     type="button"
                                                     onClick={() => updateDayShift(ds.day, 'type', t.id)}
                                                     className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${ds.type === t.id ? t.color : 'bg-slate-50 text-slate-300 filter grayscale opacity-50'}`}
                ...
                <div className="flex justify-end gap-3 mt-12 bg-gray-50 -mx-8 -mb-8 p-10 border-t border-gray-100">
                  ...
                  <button type="submit" className="px-10 py-3 bg-primary-600 hover:bg-primary-700 text-white rounded-2xl text-xs font-black shadow-lg shadow-primary-100 transition-all transform hover:scale-[1.05] active:scale-95">{newDoctor.id ? (isAr ? 'حفظ التعديلات' : 'Commit Changes') : (isAr ? 'اعتماد الطبيب' : 'Finalize Registration')}</button>
                </div>
            </form>
          </div>
        </div>
